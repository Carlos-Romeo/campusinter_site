...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$ciSession->validate($tokenName, $tokenValue)) {
        $ciNlStatut = 'err';
        $ciNlMsg    = 'Session expirée. Rafraîchissez la page et réessayez.';
    }
    /* --- 2. Rate limiting (5 req / min / IP) --- */
    elseif ($ciNlStatut === '') {
        $ip = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
        $rateKey = 'ci_rate_nl_' . md5($ip);
        $rate = (int) $ciSession->get($rateKey, 0);
        if ($rate >= 5) {
            $ciNlStatut = 'err';
            $ciNlMsg    = 'Trop de requêtes. Réessayez dans 1 minute.';
        } else {
            $ciSession->set($rateKey, $rate + 1);
        }
    }
    /* --- 3. Honeypot --- */
    if ($ciNlStatut === '' && trim((string) ($_POST['website'] ?? '')) !== '') {
        $ciNlStatut = 'ok';
